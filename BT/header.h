...
c---------------------------------------------------------------------
      ...
      pointer(ptr_bb, bb)
      pointer(ptr_cc, cc)
      pointer(ptr_BLOCK_SIZE, BLOCK_SIZE)
      ...
      pointer(ptr_elapsed_time, elapsed_time)
 
      common /global/   ptr_elapsed_time, ptr_ncells, ptr_grid_points
     ...
     >                   ptr_ty2, ptr_ty3, ptr_tz1, ptr_tz2, ptr_tz3,
     >                  ptr_dx1, ptr_dx2, ptr_dx3, ptr_dx4, 
     >                  ptr_dx5, ptr_dy1, ptr_dy2, ptr_dy3, ptr_dy4, 
     >                  ptr_dy5, ptr_dz1, ptr_dz2, ptr_dz3, ptr_dz4, 
     ...
     >        start      (3,maxcells), end      (3,maxcells)
      ...
      common /partition/ ptr_cell_coord, ptr_cell_low, ptr_cell_high,
     >                   ptr_cell_size,
     >                   ptr_grid_size, ptr_successor, ptr_predecessor, 
     >                   ptr_slice,
     >                   ptr_start, ptr_end
     ...
     >                 ue(-2:MAX_CELL_DIM+1,5), buf(-2:MAX_CELL_DIM+1,5)
      ...
      common /work_1d/ ptr_cv, ptr_rhon, ptr_rhos, ptr_rhoq, 
     >                 ptr_cuf, ptr_q, ptr_ue, ptr_buf
     ...
     >             ptr_start_send_south,
     >             ptr_start_send_north, ptr_start_send_bottom, 
     >             ptr_start_send_top,
     >             ptr_start_recv_west, ptr_start_recv_east, 
     >             ptr_start_recv_south,
     >             ptr_start_recv_north, ptr_start_recv_bottom, 
     >             ptr_start_recv_top

      double precision  tmp_block(5,5), b_inverse(5,5), tmp_vec(5)
      pointer(ptr_tmp_block, tmp_block)
      pointer(ptr_b_inverse, b_inverse)
      pointer(ptr_tmp_vec, tmp_vec)

      common /work_solve/ ptr_tmp_block, ptr_b_inverse, ptr_tmp_vec
...
c
     ...
     $        combined_ftype, idump_sub, rd_interval
      pointer(ptr_collbuf_nodes, collbuf_nodes)
      pointer(ptr_collbuf_size, collbuf_size)
      pointer(ptr_iosize, iosize)
      pointer(ptr_eltext, eltext)
      pointer(ptr_combined_btype, combined_btype)
      pointer(ptr_fp, fp)
      pointer(ptr_idump, idump)
      pointer(ptr_record_length, record_length)
      pointer(ptr_element, element)
      pointer(ptr_combined_ftype, combined_ftype)
      pointer(ptr_idump_sub, idump_sub)
      pointer(ptr_rd_interval, rd_interval)
 
      common /btio/ ptr_collbuf_nodes, ptr_collbuf_size, 
     $              ptr_iosize, ptr_eltext,
     $              ptr_combined_btype, ptr_fp, 
     $              ptr_idump, ptr_record_length,
     $              ptr_idump_sub, ptr_rd_interval

      double precision sum(niter_default), xce_sub(5)
      pointer(ptr_sum, sum)
      pointer(ptr_xce_sub, xce_sub)
  
      common /btio/ ptr_sum, ptr_xce_sub

      integer*8 iseek
      pointer(ptr_iseek, iseek)
      common /btio/ ptr_iseek, ptr_element, ptr_combined_ftype
